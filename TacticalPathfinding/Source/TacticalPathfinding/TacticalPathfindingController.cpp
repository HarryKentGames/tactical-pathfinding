...
void UTacticalPathfindingController::RunPathfinding(int startIndex, int endIndex, TArray<TacticalInformation*> tacticalInformations)
{
	if (startIndex >= 0 && startIndex < graph.Num() && endIndex >= 0 && endIndex < graph.Num())
	{
		...
		TArray<const UGraphNode*> path = UTacticalPathfinder::FindTacticalPath(graph, graph[startIndex], graph[endIndex], new EuclideanDistance(graph[endIndex]), tacticalInformations, visitedNodes);;
...
void UTacticalPathfindingController::DrawNodes(TArray<const UGraphNode*> path, FColor color, bool connect)
{
